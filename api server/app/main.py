    ...
    try:
        ...
        if not root_role:
            root_role = crud.create_role(
                db,
                schemas.RoleCreate(name="root_admin", is_admin=True, is_root=True),
            )
        admin_role = crud.get_role_by_name(db, "admin")
        if not admin_role:
            admin_role = crud.create_role(db, schemas.RoleCreate(name="admin", is_admin=True, is_root=False))
        user_role = crud.get_role_by_name(db, "user")
        if not user_role:
            user_role = crud.create_role(db, schemas.RoleCreate(name="user", is_admin=False, is_root=False))
        ...
        if root_email and root_password:
            ...
            if not existing:
                u_in = schemas.UserCreate(email=root_email, password=root_password)
